...
The product-specific methodology on this page is complemented by two
machine-readable, reusable catalogs for other intelligence applications:

- [News & Editorial Feed Catalog](/news-editorial-feed-catalog) — the union of
      configured client and server RSS/Atom feeds, including Google News wrappers,
      language tags, and variant/category membership.
- [Structured & Real-Time API Catalog](/structured-realtime-api-catalog) —
      public or reusable providers grouped by intelligence domain, without
      WorldMonitor proxy routes, credentials, or private relay configuration.

The JSON exports are committed under `shared/data-source-catalogs/` and are
generated with `npm run docs:data-source-catalogs`.
